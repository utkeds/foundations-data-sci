...
* The average score for my home state of `r naep_data_clean %>% filter(jurisdiction == "Illinois") %>% pull(jurisdiction)` is `r naep_data_clean %>% filter(jurisdiction == "Illinois") %>% pull(average_score_0_500)`.

Using code to display results reduces errors (no manual copying and pasting) and enhances reproducibility (others can see the steps you took to obtain your results). Can you use this feature to answer the questions below using code?

* The state with the maximum average score is _______________ with an average score of _______________.
* The state with the maximum average score is _______________ with an average score of _______________.
