...
Run the code chunk below to see a representation of tidy data that follows from applying two simple rules:
...
You must specify both the x- and y- variables when using `geom_point()`. 

```{r}
ggplot(joined_left, aes(x = PK12ed, y = )
```
...
We encourage you to change the theme, color, and other aesethetics of the plot. Do so by copying your code above and extending your graph.
