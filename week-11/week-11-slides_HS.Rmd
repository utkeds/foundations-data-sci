...
## Final project plans (Due Nov 2)

* I'll ask you to share some _initial, preliminary ideas_ (not final plans!) to receive early feedback (and to get your gears turning regarding what you will do): Instructions in Canvas [(link)](https://utk.instructure.com/courses/239366/discussion_topics/1792393?module_item_id=5687348)
* Please review my comments on your final project plans (I'll leave my comments by __Nov 3__)
* If you do not have the data, please obtain it as soon as possible (only a month left!)
...
```{r}
#| echo: true
glimpse(elsi_data %>% select(state_name_public_school_2021_22))
```
...
```{r}
#| echo: true
# Use str_to_upper() or str_to_lower() to standardize 
# the names before grouping
elsi_data <- elsi_data %>% 
    mutate(state_name_public_school_latest_available_year = 
               str_to_upper(state_name_public_school_latest_available_year))
```
---
...
ggplot(data = state_map) +
  geom_map(map = state_map,
    ...
    coord_fixed(1.3) +
    theme(legend.position = "none") # remove the legend
    # "bottom" or "right" will add the legend on the plot
```
]
]
...
.panel[.panel-name[Create world choropleth map]
...
.panelset[
.panel[.panel-name[Create USA choropleth map]
...
.panelset[
.panel[.panel-name[USA choropleth map with points]
...
bear_dat <- read_csv("bears.csv") 
            # Data should be placed in the same working directory 
glimpse(bear_dat)
```
...
.panel[.panel-name[Final Project Plans]

- In this assignment, you will read a bit about the final project, and share some _initial, preliminary ideas_ (not final plans!) to receive early feedback (and to get your gears turning regarding what you will do): Instructions in Canvas (https://utk.instructure.com/courses/239366/discussion_topics/1792393?module_item_id=5687348)
- It is due on Nov 2, Sunday 11:59 pm
