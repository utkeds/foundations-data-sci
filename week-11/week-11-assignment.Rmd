...
2. Let's check out how the US' per capita electricity has changed over the past few years. Filter the `owid_energy` dataset to `year > 1984` and `country == "United States"` and use `group_by()` and `summarize()` to calculate the average per capita electricity. 
...
To simplify things, let's just focus on a few mismatches. We can use a special function called `case_when()` together with `mutate()` to change the names of these columns. If you would like to learn more about `case_when()`, check the documentation: <https://dplyr.tidyverse.org/reference/case_when.html>
